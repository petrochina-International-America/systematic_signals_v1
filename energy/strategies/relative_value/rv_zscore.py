...
def _build_spread(
    ...
    """
    Stitched spread LEVEL on the intersection of both legs' trading
    calendars, in $/BBL equivalent — for charting/display ONLY. Rolling
    signals must read _build_signal_spread below: this series jumps at every
    roll/relabel and the jumps are persistently signed, biasing any trailing
    mean/std (Pass 8, notes/pass8_signal_series_integrity_2026-07-14.md).
    ...
    (the 2026-07-08 audit's blend-engine defect).
    """
...
def _build_signal_spread(
    leg1_df: pd.DataFrame,
    leg2_df: pd.DataFrame,
) -> pd.Series:
    """
    Clean signal-input spread: cumulative sum of the two legs' roll-aware
    daily flows (leg1 - leg2), anchored at the first stitched level so it
    starts on the quoted scale. No relabel jump ever enters, so trailing
    means/stds computed on it are unbiased by the roll cycle. The anchor is
    display-only — z-scores are invariant to it.
    """
    level = _build_spread(leg1_df, leg2_df, 1.0, 1.0)
    idx = level.index
    flow = (leg1_df["daily_pnl"].reindex(idx).fillna(0.0)
            - leg2_df["daily_pnl"].reindex(idx).fillna(0.0))
    cum = flow.cumsum()
...
# ============================================================
# SIGNAL — ROLLING-MEAN BAND (Z-SCORE OR ABSOLUTE-DOLLAR UNITS)
# ============================================================
def _build_signal(
    spread: pd.Series,
    lookback: int = 60,
    ...
    band_mode: str = "zscore",
) -> pd.DataFrame:
    """
    Entry when the spread's deviation from its rolling mean exceeds the
    threshold, measured in the band_mode's units:

      band_mode="zscore" (default): deviation / rolling_std vs threshold (σ).
      band_mode="dollar": raw deviation vs threshold ($/bbl — the
          Bouchouev/Zuo convention). Same lookback, same clean signal
          series, same direction logic and exit state machine; ONLY the
          units of the entry/exit comparison change. exit_threshold is
          then also in $/bbl.

    Exit behaviour controlled by exit_threshold (in band_mode units):
      exit_threshold=None (default): exit when spread crosses rolling mean
          (original state-machine behaviour).
      exit_threshold=0.0: exit when the deviation crosses zero — equivalent
          to the legacy contemporaneous rule (flat whenever |dev| < entry
          threshold, positioned whenever |dev| > entry threshold).
      exit_threshold=k (0 < k < zscore_threshold): exit when |dev| drops back
          below k — a band-narrowing exit that holds longer than zero-cross
          but exits before the opposite band is hit.

    Signal convention:
    +1 = long spread  (long leg1 / short leg2)
    -1 = short spread (short leg1 / long leg2)
    """
    if band_mode not in ("zscore", "dollar"):
        raise ValueError(f"band_mode must be 'zscore' or 'dollar', got {band_mode!r}")

    s = pd.Series(spread).astype(float)

    spread_mean = s.rolling(window=lookback, min_periods=lookback).mean()
    spread_std  = s.rolling(window=lookback, min_periods=lookback).std()
    deviation   = s - spread_mean
    denom       = spread_std.replace(0, np.nan)
    deviation_pct = deviation / denom  # z-score (kept for display in both modes)

    if band_mode == "dollar":
        entry_metric = deviation                       # $/bbl
        upper_band = spread_mean + zscore_threshold    # threshold in $
        lower_band = spread_mean - zscore_threshold
    else:
        entry_metric = deviation_pct                   # σ
        upper_band = spread_mean + zscore_threshold * spread_std
        lower_band = spread_mean - zscore_threshold * spread_std
    ...
    for i, dt in enumerate(s.index):
        ...
        prev_dev     = entry_metric.loc[prev_dt]

        if pd.isna(prev_spread) or pd.isna(prev_mean) or pd.isna(prev_dev):
            signal_raw.loc[dt] = state
            continue

        if state == 0.0:
            if prev_dev >= zscore_threshold:
                state = -1.0
            elif prev_dev <= -zscore_threshold:
                state = 1.0
            ...
            # Threshold-based exit: go flat when |dev| drops back inside exit_threshold
    ...
    out["deviation_pct"] = deviation_pct  # z-score
...
def _capital_loop(
    ...
    vol_scalar_cap / warmup_returns share the _stat_arb_capital_loop
    conventions (one sizing methodology across every pair engine): the cap
    is a hard ceiling on the low-vol leverage mechanism; the warmup seeds
    the vol estimator with pre-sample PAIR returns in the same space as the
    in-sample stream (0.5*leg_flow/prev held price per leg, roll-aware).
    """
...
def rv_zscore(
    ...
    """
    MTM stat-arb using a rolling-mean band as the entry signal.

    Entry:  |deviation from rolling mean| > zscore_threshold, where the
            deviation is measured in band_mode units:
              band_mode="zscore" (default) — deviation / rolling_std (σ).
              band_mode="dollar" — raw deviation in $/bbl (Bouchouev/Zuo
                  convention; RESEARCH mode — a fixed $ band fires more
                  often when vol rises, see the 2026-07-15 regime test).
            Both modes read the SAME clean flow-cumsum signal series.
    Exit:   controlled by exit_threshold, in band_mode units
            (see _build_signal docstring).
            exit_threshold=None  → exit when spread crosses rolling mean (default).
            exit_threshold=0.0   → exit when the deviation crosses zero (matches legacy).
            exit_threshold=k     → exit when |dev| drops below k.
    ...
    In zscore mode a threshold of 1.0 = 1 standard deviation — same
    interpretation across all pairs regardless of price level or units. In
    dollar mode the threshold is in the spread's own $/bbl units.
    trade_start : see rv_pct_deviation for semantics.
    roll_config : key in CONTRACT_SPECS for the execution roll path.
    signal_roll_config : if provided, compute the entry/exit signal from this
        roll path instead of roll_config.  When None (default) the same roll
    ...
    # Signal reads the CLEAN flow-cumsum spread (no relabel jumps); the
    # stitched level is kept for charting only (Pass 8 fix, 2026-07-14).
    signal_spread = _build_signal_spread(sig_leg1_df, sig_leg2_df)
    spread        = _build_spread(leg1_df, leg2_df, 1.0, 1.0)
    signal_df = _build_signal(signal_spread, lookback, zscore_threshold,
                              exit_threshold, band_mode=band_mode)
    pair_pnl  = _build_pair_pnl(leg1_df, leg2_df)

    full_idx = signal_df.index.intersection(pair_pnl.index).sort_values()
    idx = full_idx
    warmup_pair_ret = None
    if trade_start is not None:
        idx = full_idx[full_idx >= pd.Timestamp(trade_start)]
        # vol warmup: pre-sample PAIR returns in the loop's own vol space
        # (0.5*leg_flow/prev held price per leg — roll-aware flows)
        pre_idx = full_idx[full_idx < pd.Timestamp(trade_start)]
        if len(pre_idx) > 1:
            px1_pre = leg1_df["held_price"].reindex(pre_idx).ffill().shift(1)
            px2_pre = leg2_df["held_price"].reindex(pre_idx).ffill().shift(1)
            warmup_pair_ret = (
                0.5 * pair_pnl["leg1_daily_pnl"].reindex(pre_idx) / px1_pre
                - 0.5 * pair_pnl["leg2_daily_pnl"].reindex(pre_idx) / px2_pre
            ).dropna()
    ...
    strategy_df = _capital_loop(
        ...
        vol_scalar_cap=vol_scalar_cap, warmup_returns=warmup_pair_ret,
    )

    for col in ["spread", "spread_mean", "spread_std", "deviation", "deviation_pct", "upper_band", "lower_band"]:
...
def rv_zscore_blend(
    ...
    """
    Consensus blend of a fast and slow z-score signal.
    ...
    trade_start : see rv_zscore for semantics.
    ...
    # Both timeframes read the CLEAN flow-cumsum spread (Pass 8 fix).
    spread        = _build_spread(leg1_df, leg2_df, 1.0, 1.0)
    signal_spread = _build_signal_spread(leg1_df, leg2_df)
    pair_pnl = _build_pair_pnl(leg1_df, leg2_df)

    fast_df = _build_signal(signal_spread, fast_lookback, fast_threshold)
    slow_df = _build_signal(signal_spread, slow_lookback, slow_threshold)
    ...
    warmup_pair_ret = None
    if trade_start is not None:
        full_idx2 = pair_pnl.index.sort_values()
        pre_idx = full_idx2[full_idx2 < pd.Timestamp(trade_start)]
        if len(pre_idx) > 1:
            px1_pre = leg1_df["held_price"].reindex(pre_idx).ffill().shift(1)
            px2_pre = leg2_df["held_price"].reindex(pre_idx).ffill().shift(1)
            warmup_pair_ret = (
    ...
    for col in ["spread", "spread_mean", "spread_std", "deviation", "deviation_pct"]:
